<x-guest-layout>
    ...
    <div class="hidden lg:flex w-1/2 relative overflow-hidden">
        <img src="{{ asset('images/ordecity-reset-background.jpg') }}" alt="Ordecity Team" class="absolute inset-0 w-full h-full object-cover" />
        ...
        <div class="relative z-10 flex flex-col justify-between h-full p-12 text-white">
            <div class="space-y-6">
                <div class="flex items-center space-x-2">
                    ...
                    <div class="text-sm tracking-wider">ACCOUNT RECOVERY</div>
                </div>
                <h1 class="text-5xl md:text-6xl font-bold leading-tight">
                    Ordecity
                    <span class="block text-2xl md:text-3xl font-light mt-2">Reset Your Password</span>
                </h1>
                <p class="text-lg md:text-xl max-w-md">
                    Create a new password for your Ordecity account and continue enhancing your organization's potential
                </p>
            </div>
    ...
    <div class="w-full lg:w-1/2 bg-gradient-to-b from-gray-50 to-white px-8 lg:px-12 flex flex-col justify-center">
        ...
        <div class="w-full max-w-md mx-auto relative z-10">
            <div class="flex items-center justify-start space-x-3 mb-8">
                ...
                <span class="text-2xl font-bold text-gray-800">Reset Password</span>
            </div>
            ...
            <div class="inline-flex items-center mb-6">
                <div class="flex items-center space-x-2 bg-white/80 backdrop-blur px-4 py-2 rounded-full shadow-xl border border-gray-200">
                    ...
                    <span class="text-gray-800 font-medium">Secure & Private Reset</span>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="space-y-6">
                ...
                <div class="space-y-1">
                    ...
                    <input id="email" type="email" name="email" :value="old('email', $request->email)" required autofocus autocomplete="username" class="block w-full px-4 py-3 rounded-lg border-gray-300 shadow-sm focus:border-[#0575E6] focus:ring focus:ring-[#0575E6]/20 focus:ring-opacity-50 transition duration-150 ease-in-out @error('email') border-red-500 @enderror">
                    @error('email')
                ...
                <div class="bg-white/80 rounded-lg p-4 text-sm text-gray-600 shadow-sm border border-gray-200">
                ...
                <button type="submit" class="group relative w-full flex justify-center py-3 px-6 border border-transparent rounded-lg shadow-xl text-sm font-medium text-white bg-[#0575E6] hover:bg-[#045dc1] transition-all duration-300 overflow-hidden animate-fade-in-up">
                    ...
                    <span class="relative flex items-center justify-center gap-2">{{ __('Reset Password') }} <i class="fas fa-arrow-right transition-transform group-hover:translate-x-1"></i></span>
                </button>

                <p class="mt-6 text-center text-sm text-gray-600">
                    Remember your password? <a href="{{ route('login') }}" class="font-medium text-[#0575E6] hover:text-[#045dc1] transition-colors duration-150 ml-1">Back to login</a>
                </p>
            </form>
        </div>
    </div>
